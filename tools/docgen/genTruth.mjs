...
<div class="ch">
  ...
  <p>서지안은 목사가 협심증을 앓는 걸 알고 있었다. 그는 자연사처럼 보이는 죽음을 계획했다. 수련회 당일 아침, 목사 방에 들어가 <strong>협심증 응급약 설하정 통의 6알을, 자신이 가져온 비타민C 6알과 바꿔치기</strong>했다. 발작이 와도 응급약이 듣지 않게 만든 것이다. (목사가 등산을 떠나 방이 빈 오전 10시 무렵의 일이다 — 복도 CCTV 에 10:10~12 로 찍혀 있다.) 빼낸 진짜 설하정은 버리지 않고 자신의 <strong>요일별 약통(월~토)</strong>에 비타민C·루테인·오메가3 같은 영양제와 섞어 숨겼다. (목사 책상의 설하정 통에는 노란 알약이 5알만 남게 되었다.)</p>
...
<div class="ch">
  ...
  <p>한다영은 목사를 완전히 죽일 생각까진 없었다. 다만 <strong>"목사가 한 번 쓰러져서 재정 점검이 미뤄지기만 하면"</strong> 하는 마음이었다. 막내 최종현이 위험한 다이어트 보충제 요힘빈을 먹는다는 걸 알고 있던 그녀는, 최종현이 등산 간 사이 그의 방에 들어가 <strong>요힘빈 통과 단백질 통의 라벨지를 바꿔놨다.</strong></p>
...
<div class="ch">
  ...
  <p>한소미는 동생과 구체적 방법을 상의하지 않고 독자적으로 움직였다. <strong>목사 책상 위에는 항상 개인 텀블러가 놓여 있었다.</strong> 그녀는 강한 수면제(졸피뎀)를 그 텀블러에 타 넣었다. 목사가 깊이 잠들어 정신을 못 차리면 재정 점검도 흐지부지될 거라 생각했다.</p>
