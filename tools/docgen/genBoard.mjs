  ...
  if (c.handwriting?.options?.length) {
    ...
    return [one({
      detail: line(c.detail,
        ...
        + '\n그 사람의 다이어리 카드가 판에 나온 뒤라야 대조할 수 있다 — 누군가 그 카드를 가져와 내용을 알렸으면 된다.'
  ...
  for (const t of allClues) {
    ...
    if (t.type === '감식' && src.length === 1) {
      push(num[src[0]], `🔬 감식실에 내면 → <b>${num[t.code]}</b>. `
        + `<span class="hnote">내는 데 조사 행동을 쓰지 않는다. 결과는 모두에게 소리 내어 알린다</span>`);
      continue;
    }
    for (const s of src) {
      ...
      push(num[s], others.length
        ? `⭐ <b>${others.join(' + ')}</b> 와 함께 → 특수 <b>${num[t.code]}</b>. `
          + `<span class="hnote">재료가 <b>모두 조사되면</b> 그 자리에서 함께 공개한다 — 가진 사람이 달라도, 합의하지 않아도 된다</span>`
        : `⭐ 이 카드 하나로 → 특수 <b>${num[t.code]}</b>. `
          + `<span class="hnote">이 카드가 조사되면 그 자리에서 공개하고 특수 더미에서 가져간다</span>`);
    }
  }
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* 표시는 다섯이다 — 🔒 못 읽는 카드, ⚖ 본인이 못 읽는 감식, 🔬 감식실에 낼 것, ⭐ 조합 재료.
...
(function () {
  /* 종이보다 5mm 짧게 잡는다 — 딱 맞춰 재면 반올림과 프린터 여백에서 한 면이 더 생긴다. */
  var MIN = 0.82, H = 292 / 25.4 * 96;
  function fit() {
...
function lockCards() {
  const face = (q) => `<div class="card" style="border-color:#5a3f8a">
    ...
    <div class="hint"><div>이 카드는 <b>아무도 가져갈 수 없다.</b> 판 옆에 펴 두고 누구나 찍는다.<br>
      ${q.hand ? `<b>${esc(q.need)}</b>가 판에 나온 뒤라야 찍을 수 있다.<br>한 라운드에 세 명까지.` : '잠긴 것은 카드가 아니라 <b>숫자</b>다.'}</div></div>
  </div>`;
  return `<div class="sheetPage"><div class="deckHd"><h1>필적 대조 카드 — ${allQ().length}장
    ...
    <p class="muted">앞면이 보이게 판 옆에 펴 둡니다. <b>가져가는 카드가 아닙니다.</b>
      ...
      <b>그 사람의 다이어리 카드가 판에 나온 뒤라야</b> 찍을 수 있습니다 —
      누군가 가져와 내용을 알렸으면 됩니다.<br>
  ...
  const cover = (s) => `<div>
    ...
    <div>
      ...
      <div class="covHow"><b>이 시트를 받으면</b>
        ...
        <div>② <b>나이가 많은 사람부터</b> 차례로 <b>겉면에 적힌 것</b>(가족·나이·직책과
          <b>「알려진 것」</b>)을 자기 말로 소개합니다.
          <span class="muted">동갑이면 직책이 위인 쪽부터 — 전도사 · 회장 · 회계 · 총무 · 팀장 · 서기 순입니다.</span>
    ...
    <div class="covFoot">한 사람에 <b>A4 가로 두 장</b>입니다. 두 장 다 양면 인쇄(<b>짧은 쪽 넘김</b>)해
      ...
      그러면 바깥으로 나오는 것은 <b>이 겉면과 「둘째 장이 따로 있습니다」, 이름표, 그리고 백지 하나</b>
      — 넉 면뿐입니다. 접고 나서 이 넷이 맞는지 세어 보세요.
  ...
  const myPhone = (who) => {
    ...
    return `<div class="box"><div class="bl">내 휴대폰 — 톡서랍 복구 번호 ${esc(num)}</div>
      <p>내 폰(<b>${esc(no(mine.code))}</b>)의 카카오톡에는 <b>내가 지운 대화방</b>이 있습니다.
        되살리려면 이 네 자리가 필요하고, <b>그 번호를 아는 사람은 나뿐이라고 믿고 있습니다.</b>
...
`;
  ...
  // 한 사람에 두 장. 바깥 넷은 겉면 ·「둘째 장이 따로 있습니다」· 이름표 · 백지 하나 —
  //   어느 쪽으로 접어도 비밀이 밖을 보지 않는다. 겉면의 자가 검사도 이 넷을 세게 해야 한다.
...
function placeBoard() {
  ...
  <div class="page"><div class="stg">이벤트 ② 를 읽은 뒤 · 판 옆에 펴 둡니다</div>
    <h1>감식실 <span class="muted">— 결과는 모두에게 알립니다</span></h1>
...
function runSheets() {
  ...
  const body = `<div class="page">${stage('제일 먼저 · 뽑는 사람만')}<h1>먼저 이렇게 뽑습니다
    <span class="muted">— 이 한 장만 먼저 뽑아 보세요</span></h1>
    <p>인쇄물은 <b>여섯 가지</b>입니다. 아래 순서대로 뽑으면 됩니다.
      <b>이 일을 맡은 사람은 플레이어이기도 합니다</b> — 그래서 순서가 중요합니다.
      답과 남의 비밀부터 먼저 치우고, 오래 걸리는 것을 나중에 합니다.</p>

    <table><tr><th style="width:6%">순</th><th style="width:26%">인쇄물</th>
      <th style="width:13%">종이</th><th style="width:9%">면</th>
      <th style="width:22%">양면 · 넘김</th><th>나오는 장수 · 그다음 할 일</th></tr>
      <tr><td class="stn">1</td><td><b>진상해설서</b></td><td>A4 세로</td><td>12</td>
        <td>단면</td>
        <td>12장. <b>혼자 있을 때 뽑으세요.</b> 배출된 채로 집어 <b>첫 면만 확인하고 봉투에 넣어 봉합니다</b> —
          둘째 장부터가 답입니다.</td></tr>
      ...
      <tr><td class="stn">6</td><td><b>보드_진행물</b></td><td>A4 세로</td><td>11</td>
        <td>단면</td>
        <td>11장(이 책자입니다). <b>첫 면은 이미 뽑았으니 2~11면만 뽑으면 됩니다</b> — 통째로 다시 뽑으면 첫 면이 한 장 더 나옵니다.<br><b>이벤트 카드 넉 장</b>이 마지막 두 면에 걸쳐 있습니다 — 넉 장을 다 오렸는지 세어 보세요.</td></tr></table>
  ...
  <div class="page">${stage('뽑은 뒤 · 자르기 전에')}<h1>뽑은 뒤 이렇게 합니다
    ...
    <p><b>① 카드 한 장을 자로 재세요. 63 × 88mm</b> 가 아니면 배율이 잘못된 것입니다.
      ...
      종이 테두리를 잘라내기 전이라 아직 큽니다.
      <b>한가운데 칸이 비어 있는 면이 두 장 있습니다</b>(카드가 석 장인 면과 한 장인 면) —
      그 둘 말고 다른 면으로 재세요.</span><br>
  ...
  <div class="page">${stage('뽑고 나서 · 처음 모였을 때')}<h1>여기서부터 <span class="muted">— 이 게임을 처음 하는 분들께</span></h1>
    ...
    <table><tr><th style="width:7%">순</th><th style="width:30%">무엇을</th><th>누가 · 어떻게</th></tr>
      ...
      <tr><td class="stn">4</td><td><b>자기소개</b><br>
        ...
        <td><b>나이가 많은 사람부터</b> 차례로. 나이는 인물 시트 <b>겉면</b>에 적혀 있습니다 —
          동갑이면 직책이 위인 쪽부터(전도사 · 회장 · 회계 · 총무 · 팀장 · 서기).<br>
          겉면에 적힌 것을 자기 말로. 한 사람에 30초.<br>
          <b>가장 먼저 소개한 사람이 1라운드 시작 플레이어입니다.</b> 그다음부터는
          <b>앉은 자리에서 시계 방향으로</b> 한 칸씩 넘어갑니다 — 조사 차례도 시계 방향입니다.</td></tr>
  ...
  <div class="page">${stage('헷갈릴 때')}<h1>이 종이는 무엇인가 <span class="muted">— 헷갈리면 여기를 보세요</span></h1>
    <table><tr><th style="width:26%">무엇</th><th>어떤 물건인가</th></tr>
      ...
      <tr><td><b>필적 대조 카드</b><br><span class="muted">Q6 일곱 장</span></td>
        <td>앞면이 보이게 펴 둡니다. <b>가져가는 카드가 아닙니다</b> — 그 자리에서 QR 만 찍습니다.<br>
          그 사람의 <b>다이어리가 판에 나온 뒤</b>(누군가 가져와 내용을 알린 뒤)라야 찍을 수 있고,
          <b>한 라운드에 세 명까지</b>입니다(조사와 별개).</td></tr>
    ...
    <table><tr><th style="width:22%">표시</th><th>뜻</th></tr>
      ...
      <tr><td><span class="lg">🔬</span></td><td>감식실에 낼 수 있는 것입니다. 결과는 <b>모두에게 알립니다</b></td></tr>
  ...
  <div class="page board">${stage('준비할 때')}<h1>탁자에 이렇게 놓습니다</h1>
    ...
    <div class="tbl">
      ...
      <div class="tblRow">
        <div class="slot slotSheet">라운드 트랙 <span>말 하나 · <b>이벤트 카드 넉 장</b>을
          접어서 <b>뒷면이 보이게</b> ①②③④ 자리에 얹어 둔다 — 트랙 위에만 두고 단서 카드와 섞지 않는다</span></div>
  ...
  <div class="page">${stage('첫 라운드 전에 · 계속 펴 둡니다')}<h1>기본 규칙 <span class="muted">— 앞장</span></h1>
    ...
    <p>①<b>조사</b> — 순서대로 한 명씩, 열려 있는 장소 <b>하나</b>를 골라 그 장소의 남은 번호 중
      ...
      &nbsp;&nbsp;조사는 <b>시작 플레이어부터 앉은 자리에서 시계 방향으로</b> 한 사람씩 합니다.
      시작 플레이어는 <b>라운드마다 한 칸씩 시계 방향으로 돕니다.</b><br>
    ...
    <p><b>고른 장소에 남은 카드가 2장에 못 미치면, 남은 만큼만 가져갑니다.</b> 한 장이면 한 장입니다 —
      ...
      토론에는 그대로 참여합니다. <b>새로 나올 것이 없다는 뜻이니, 그 라운드는 지금까지 나온 것을
      맞춰 범인을 찾는 데 씁니다.</b><br>
  ...
  <div class="page">${stage('첫 라운드 전에 · 계속 펴 둡니다')}<h1>기본 규칙 <span class="muted">— 뒷장</span></h1>
    ...
    <table><tr><th style="width:20%">표시</th><th>뜻</th></tr>
      ...
      <tr><td><span class="lg">⭐</span> <b>조합 재료</b></td>
        <td>다른 카드와 함께 모으면 <b>특수 단서(S)</b> 를 가져갑니다. 무엇과 묶는지는 카드 아래에 적혀 있습니다.
          재료가 <b>모두 조사되면</b> 그 자리에서 함께 공개합니다 — 합의하지 않아도 됩니다.</td></tr>
    ...
    <h2>「판에 나온다」와 「공개한다」</h2>
    <p><b>카드는 남에게 보여 주지 않습니다.</b> 가져온 카드는 혼자 읽고 그 내용을 <b>말로</b> 전합니다.
      보태도 되고, 빼도 되고, 지어내도 됩니다 — 그럴 수 있어야 이 게임입니다.<br>
      <b>「그 카드가 판에 나왔다」</b>는 것은 <b>누군가 그 카드를 가져와 내용을 말로 알린 것</b>을 뜻합니다.
      번호만 남 앞에 놓여 있는 것은 나온 것이 아닙니다 — 무엇을 몇 장 가졌는지는 원래 서로 보입니다.</p>
    <p><b>딱 하나, 조합(⭐)만은 실제로 내보입니다.</b> 조합을 이루려면 재료가 되는 카드들을
      <b>판 가운데에 앞면으로 내려놓아 모두가 직접 읽게 해야</b> 합니다 — 이것을 <b>공개</b>라고 합니다.
  ...
  <div class="page">${stage('첫 라운드 전에 · 계속 펴 둡니다')}<h1>기본 규칙 <span class="muted">— 셋째 장</span></h1>
    ...
    <p>1. 채취물을 가진 사람이 라운드 끝에 그 카드를 <b>앞면으로 감식실 옆에 내려놓습니다.</b>
      ...
      2. 다음 라운드 시작 때 그 결과 번호(L…)를 집어 <b>모두에게 소리 내어 알립니다.</b>
      혼자 읽고 덮어 두지 못합니다 — 감식 결과는 판 전체가 함께 아는 것입니다.
      읽은 뒤 그 결과 카드는 읽은 사람이 갖고,
    ...
    <p class="muted">결과를 읽는 손과 결과가 걸린 목이 같으면 그 카드는 증거가 아니라 증언이 됩니다.
      ...
      그 카드가 영영 잠기므로, 넘겨서 내는 길을 열어 둡니다.</p>

    <h2>특수 단서</h2>
    <p>조합(⭐)의 <b>재료가 되는 카드가 모두 조사되면</b> — 누구 손에 있든 —
  ...
  <div class="page board">${stage('라운드마다')}<h1>라운드 트랙 <span class="muted">— 두 표가 한 면에 있습니다. 인원에 맞는 쪽만 보세요</span></h1>
    ...
    <p class="muted"><b>여섯이면 6라운드, 일곱이면 5라운드입니다.</b>
      일곱은 한 라운드에 조사가 일곱 번 돌아, 라운드가 하나 적어도 조사 횟수는 거의 같습니다.<br>
      뒤로 갈수록 장소마다 남는 카드가 줄어듭니다 — 모자라면 <b>세 명 제한을 풀고</b> 나눠 가집니다.<br>
  ...
  <div class="page spill">${stage('준비할 때 오려 둡니다')}<h1>이벤트 카드 <span class="muted">— 잘라서 접어 두세요</span></h1>
    ...
    ${ev('②', whenEv('②'), '유품 반출 동의 · 통신 기록 영장',
      ...
      <p><b>감식실(L)도 함께 열립니다.</b> <b>이 카드를 읽는 지금 바로</b> 채취물을 낼 수 있습니다 — 다음 라운드까지 기다리지 않습니다.<br>
        ...
        결과는 다음 라운드에 <b>모두에게</b> 소리 내어 알립니다.</span></p>`)}
  </div>

  <div class="page">${stage('준비할 때 오려 둡니다 · 이어짐')}<h1>이벤트 카드 <span class="muted">— ③ 과 ④</span></h1>
    ...
    ${ev('④', whenEv('④'), '2차 부검 소견 — 타살로 확정',
      ...
      <p><b>아직 감식실에 안 낸 채취물(🔬)이 있으면 남은 라운드 안에 내세요.</b>
        마지막 라운드에 낸 것도 <b>최종 토론이 시작될 때 읽습니다</b> — 늦게라도 내는 것이 안 내는 것보다 낫습니다.
        <b>거기까지 안 내면 그 감식 결과는 판에 나오지 않은 채로 끝납니다.</b>
        누가 무엇을 안 냈는지는 서로 보이니, 안 내는 것도 하나의 대답입니다.<br>
