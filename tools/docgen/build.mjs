...
import { lint, allClues, suspects, victim, recover } from './loadData.mjs';
...
async function main() {
  ...
  const docs = [...genAllRefSheets(), genPlaceGuide(), genTruth({ victim }), ...(await genQRDocs()), genResultSheet(), ...(await genBoardDocs({ allClues, suspects, recover }))];
